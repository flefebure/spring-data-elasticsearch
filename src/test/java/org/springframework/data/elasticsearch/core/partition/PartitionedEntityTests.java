...
public class PartitionedEntityTests {
	...
	public void testSearchQueries() throws ParseException {
		...
		Page<LongPartitionedEntity> page = longPartitionedEntityRepository.search(builder.build());
		...
		builder.withQuery(QueryBuilders.matchAllQuery());
		Page<DatePartitionedEntity> dateFacetedPage = datePartitionedEntityRepository.search(builder.build());
	...
	public void testCompositePartition() throws ParseException {
		...
		builder.withPartition(new DatePartition("creationDate", sdf.parse("01/02/2015"), sdf.parse("01/07/2015")));
		builder.withQuery(QueryBuilders.matchAllQuery());
		Page<CompositePartitionedEntity> page = compositePartitionedEntityRepository.search(builder.build());
		Assert.assertEquals(1, page.getContent().size());
